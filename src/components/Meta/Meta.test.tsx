...
describe('<Meta />', () => {
  describe('Dashboard route', () => {
    test('It renders correctly', () => {
      ...
      expect(wrapper.find('link[rel="canonical"]').prop('href')).toEqual(
        'https://explorer.ironfish.network/dashboard',
      )
    })
  })
  ...
  describe('TransactionDetailPage route', () => {
    test('It renders correctly', () => {
      const wrapper = shallow(
        <Meta path={RoutePath.TransactionDetailPage} variables={{ hash: 'thisisthehash' }} />,
      )
      expect(wrapper.find('title').text()).toEqual(
        'app.transactionDetailPage.meta.titlethisisthehash - app.meta.defaultTitle',
      )
      expect(wrapper.find('meta[name="description"]').prop('content')).toEqual(
        'app.transactionDetailPage.meta.descriptionthisisthehash',
      )
      expect(wrapper.find('meta[name="keywords"]').prop('content')).toEqual(
        'app.meta.defaultKeywords',
      )
      expect(wrapper.find('link[rel="canonical"]').prop('href')).toEqual(
        'https://explorer.ironfish.network/transactions/thisisthehash',
      )
    })
  })
})
